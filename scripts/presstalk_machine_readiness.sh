...
if [[ -f "$STATUS_JSON" ]]; then
  ...
  asr_backend="$(status_value runtime.asrBackend)"
  streaming_asr_backend="$(status_value runtime.streamingASRBackend)"
  asr_mode="$(status_value runtime.asrMode)"
  realtime_partial_transcription_enabled="$(status_value runtime.realtimePartialTranscriptionEnabled)"
...
print_text_report() {
  ...
  if [[ "$runtime_status_available" == "true" ]]; then
    ...
    print_field "ASRBackend" "$asr_backend"
    print_field "StreamingASRBackend" "$streaming_asr_backend"
    print_field "ASRMode" "$asr_mode"
    print_field "RealtimePartialTranscriptionEnabled" "$realtime_partial_transcription_enabled"
...
write_json_report() {
  ...
  plist_insert_string "$tmp_plist" "runtime.asrBackend" "$asr_backend"
  plist_insert_string "$tmp_plist" "runtime.streamingASRBackend" "$streaming_asr_backend"
  plist_insert_string "$tmp_plist" "runtime.asrMode" "$asr_mode"
  plist_insert_bool_or_string "$tmp_plist" "runtime.realtimePartialTranscriptionEnabled" "$realtime_partial_transcription_enabled"
