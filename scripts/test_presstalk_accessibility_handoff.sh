...
grep -Fq 'if [[ ! -d "$APP_BUNDLE" && -d "$HOME/Applications/PressTalk.app" ]]; then' "$fixture_command"
grep -Fq "HELPER_NAME='presstalk_accessibility_handoff.sh'" "$fixture_command"
grep -Fq 'HELPER_PATH="$APP_BUNDLE/Contents/Resources/$HELPER_NAME"' "$fixture_command"
grep -Fq 'HELPER_PATH="$APP_BUNDLE/Contents/Resources/$CANONICAL_HELPER_NAME"' "$fixture_command"
