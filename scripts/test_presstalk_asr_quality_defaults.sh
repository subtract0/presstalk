...
RECALL_SOURCE="$REPO_ROOT/Sources/PressTalkCore/TranscriptRecallPolicy.swift"
CONFIG_SOURCE="$REPO_ROOT/Sources/JarvisTap/JarvisTapConfig.swift"
...
require_contains "$CONFIG_SOURCE" 'env["PRESSTALK_PARAKEET_QUALITY_FALLBACK"]' "runtime must read Parakeet quality fallback setting"
require_contains "$CONFIG_SOURCE" 'env["PRESSTALK_PARAKEET_MIN_CONFIDENCE"]' "runtime must read Parakeet confidence threshold"
...
require_contains "$RECALL_SOURCE" "Parakeet v3 ANE transcript accepted but ends like a truncated prefix" "runtime must reject dangling high-confidence Parakeet endings when streaming saw more speech"
require_contains "$RECALL_SOURCE" "Whisper candidate deferred because it is much shorter than accepted Parakeet recall candidate" "runtime must detect truncated Whisper fallback candidates"
require_contains "$RECALL_SOURCE" "Whisper candidate deferred because it is much shorter than streaming recall candidate" "runtime must detect mixed-language tail-only Whisper candidates"
require_contains "$RECALL_SOURCE" "Whisper candidate rejected because it loses recall prefix context=" "runtime must reject short Whisper fallbacks that lost the recalled prefix"
require_contains "$RECALL_SOURCE" "Whisper candidate rejected because it loses recall prefix and looks repetitive" "runtime must reject repetitive Whisper fallbacks that lost the recalled prefix"
require_contains "$RECALL_SOURCE" "Whisper candidate rejected because it loses recall prefix and ends mid-phrase" "runtime must reject dangling Whisper fallbacks that lost the recalled prefix"
require_contains "$SOURCE" "Chunked offline Whisper fallback completed" "runtime must retry truncated mixed-language Whisper candidates in chunks"
require_contains "$SOURCE" "Using chunked offline Whisper transcript after short-candidate recall deferral" "runtime must prefer chunked fallback when it recovers deferred mixed-language speech"
require_contains "$SOURCE" '"asrBackend": status.asrBackend' "runtime status JSON must record the active ASR backend"
require_contains "$SOURCE" '"streamingASRBackend": status.streamingASRBackend' "runtime status JSON must record the live partial ASR backend"
...
require_contains "$SOURCE" 'parakeet_v3_ane_final_pass_with_\(streamingMode)_true_streaming_partials' "runtime must expose the hybrid Parakeet ANE finalizer plus true-streaming partial mode"
...
require_contains "$BOOTSTRAP" 'PRESSTALK_STREAMING_ASR_BACKEND="${PRESSTALK_STREAMING_ASR_BACKEND:-${JARVISTAP_STREAMING_ASR_BACKEND:-parakeet-eou-320}}"' "bootstrap must default live partials to Parakeet EOU 320"
require_contains "$BOOTSTRAP" 'PRESSTALK_ENABLE_STREAMING_TRANSCRIPTION="${PRESSTALK_ENABLE_STREAMING_TRANSCRIPTION:-${JARVISTAP_ENABLE_STREAMING_TRANSCRIPTION:-1}}"' "bootstrap must default streaming transcription on"
...
require_contains "$INSTALLER" 'PRESSTALK_STREAMING_ASR_BACKEND="${PRESSTALK_STREAMING_ASR_BACKEND:-${JARVISTAP_STREAMING_ASR_BACKEND:-parakeet-eou-320}}"' "LaunchAgent installer must default live partials to Parakeet EOU 320"
require_contains "$INSTALLER" 'PRESSTALK_ENABLE_STREAMING_TRANSCRIPTION="${PRESSTALK_ENABLE_STREAMING_TRANSCRIPTION:-${JARVISTAP_ENABLE_STREAMING_TRANSCRIPTION:-1}}"' "LaunchAgent installer must default streaming transcription on"
