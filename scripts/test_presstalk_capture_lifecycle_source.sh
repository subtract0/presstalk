...
if rg -q --fixed-strings 'lowercasedName.contains("shure")' "$DEVICE_SOURCE" ||
   rg -q --fixed-strings 'lowercasedName.contains("mv7")' "$DEVICE_SOURCE"; then
  echo "FAIL: microphone fallback scoring must not hard-code user-specific hardware names"
  exit 1
fi
...
require_contains "let liveInputDeviceID = selectedAudioInput?.isDefault == true ? nil : selectedAudioInput?.id" "The macOS default input must use AVAudioEngine's default path instead of explicit device reassignment"
require_contains "startRecordingLive(inputDeviceID: liveInputDeviceID)" "Recording must use the resolved input-device ID path"
