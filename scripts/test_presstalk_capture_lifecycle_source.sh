...
require_contains "safelyStopLiveAudioRecording(whisperKit: whisperKit, reason: \"capture_stall_fallback\")" "Stalled explicit-device capture must be torn down before falling back to the system default input"
...
require_contains "Live audio capture stalled selected_input=" "Explicit-device callback starvation must be logged before fallback"
require_contains "try whisperKit.audioProcessor.startRecordingLive(inputDeviceID: nil)" "Stalled explicit-device capture must retry the macOS default input"
