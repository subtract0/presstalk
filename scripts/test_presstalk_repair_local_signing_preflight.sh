...
trusted_local_signing_output="$TEST_TMPDIR/trusted-local-signing.txt"
HOME="$home_dir" PRESSTALK_APP_BUNDLE="$app_bundle" PRESSTALK_TEST_EXISTING_IDENTITY=ready "$HELPER" --preflight >"$trusted_local_signing_output"
grep -Fq "RepairNeeded: false" "$trusted_local_signing_output"
grep -Fq "RepairAllowedHere: true" "$trusted_local_signing_output"
grep -Fq "WouldRunRepair: false" "$trusted_local_signing_output"
grep -Fq "SigningTrustPromptNeeded: false" "$trusted_local_signing_output"
grep -Fq "ExistingSigningIdentity: ready" "$trusted_local_signing_output"
grep -Fq "NextAction: No signing repair needed for the current runtime status." "$trusted_local_signing_output"
...
grep -Fq "Trigger: fn" "$default_trigger_output"
...
grep -Fq -- "--trigger-key 'fn'" "$desktop_command"
