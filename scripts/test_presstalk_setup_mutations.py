...
cases = [
    ...
    ('shortcut_selection', ui, 'onChangeShortcut?(shortcutChoices[sender.indexOfSelectedItem].id)', 'PRESSTALK_TEST_SETUP_UI_SOURCE',
     'Setup shortcut selection did not call its production action', ['bash', 'scripts/test_presstalk_setup_controls.sh']),
    ('shortcut_persistence', main, 'self.settingsStore.triggerKey = shortcut', 'PRESSTALK_TEST_APP_SOURCE',
     'Setup shortcut callback did not update the actual settings store', app_command),
]
with tempfile.TemporaryDirectory(prefix='presstalk-setup-mutations-') as directory:
    def run(case):
        ...
        path.write_text(source.replace(call, 'false' if name == 'shortcut_selection' else '/* deliberately disconnected */', 1))
...
print(f'PASS: all {len(cases)} removed production calls fail behavioral tests')
